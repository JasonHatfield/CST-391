...
export class EditAlbumComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.service.getAlbum(artist!, Number.parseInt(id!), (album: Album | null) => {
      if (album) {
        this.album = album;
        for (let x = 0; x < this.album.Tracks.length; ++x) {
          this.tracksRaw = this.tracksRaw + this.album.Tracks[x].Title;
          if (
            this.album.Tracks[x].Lyrics != null &&
            this.album.Tracks[x].Lyrics != ''
          )
            this.tracksRaw = this.tracksRaw + ';' + this.album.Tracks[x].Lyrics;
          if (
            this.album.Tracks[x].Video != null &&
            this.album.Tracks[x].Video != ''
          )
