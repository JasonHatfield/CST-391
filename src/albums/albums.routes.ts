...
const router = Router();
/*
 * In the albums.routes.ts file, the router defines the route for the specific endpoint. 
 * For our example, the route is defined as: router.get('/albums', AlbumsController.readAlbums);
 * The "readAlbums" function will be called when a client sends a GET request to the '/albums' 
 * endpoint.
 */
router
  .route('/albums')
